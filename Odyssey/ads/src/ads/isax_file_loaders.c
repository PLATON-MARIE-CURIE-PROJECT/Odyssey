...
void greedy_query_scheduling_chatzakis(dress_query *queries, dress_query *queries_to_ans, int *queries_counter, int q_num, greedy_alg_comparing_field cmp_field)
{
    ...
    double node_score[/*node_group_total_nodes*/ comm_sz];
...
void print_dress_queries_chatzakis(dress_query *queries, int q_num)
{
    ...
    for (int i = 0; i < q_num; i++)
    {
        ...
        printf("[%d]->([%d], %f)\n", i, queries[i].id, queries[i].initial_estimation);
    }
...
{
    parallel_query_thread_data *data = (parallel_query_thread_data *)arg;
    ts_type *paa = malloc(sizeof(ts_type) * data->index->settings->paa_segments);

    int real_distances = 0, min_distances = 0, queries_ans = 0;
    struct timeval t1, t2;
    struct timeval t1R, t2R;
    double elapsedTime, elapsedTimeR;

    printf("[MC] - Running thread id %d \n", data->tid);

    // start timer
    gettimeofday(&t1, NULL);

    for (int q_loaded = data->from; q_loaded < data->to; q_loaded++)
    {
        // printf("[MC] Thread %d -- Answering query with ID %d\n", data->tid,q_loaded);

        paa_from_ts(data->queries[q_loaded], paa, data->index->settings->paa_segments, data->index->settings->ts_values_per_paa_segment);

        // COUNT_TIME_PASSED_START
        gettimeofday(&t1R, NULL);
        query_result result = data->search_function(data->queries[q_loaded], paa, data->index, &(data->nodelist), data->minimum_distance, q_loaded, &min_distances, &real_distances);
        gettimeofday(&t2R, NULL);
        elapsedTimeR = (t2R.tv_sec - t1R.tv_sec) * 1000.0;    // sec to ms
        elapsedTimeR += (t2R.tv_usec - t1R.tv_usec) / 1000.0; // us to ms
        // printf("Time needed for thread QA %d is %f s. for single result...\n", data->tid, elapsedTimeR / 1000);

        // COUNT_TIME_PASSED_END

        data->results[q_loaded] = result.distance;

        double time_for_the_query = elapsedTimeR;
        queries_times[q_loaded] = time_for_the_query;

        queries_statistics_manol[q_loaded].total_time = time_for_the_query;
        queries_statistics_manol[q_loaded].final_bsf = result.distance;

        queries_statistics_manol[q_loaded].lower_bound_distances = min_distances;
        queries_statistics_manol[q_loaded].real_distances = real_distances;

        data->for_all_queries_real_dist_counter_perthread += real_distances;
        data->for_all_queries_min_dist_counter_perthread += min_distances;
    ...
    elapsedTime = (t2.tv_sec - t1.tv_sec) * 1000.0;    // sec to ms
    elapsedTime += (t2.tv_usec - t1.tv_usec) / 1000.0; // us to ms
    printf("Time needed for thread QA %d is %f s for answering %d queries.\n", data->tid, elapsedTime / 1000, queries_ans);

    free(paa);
    pthread_exit(NULL);
}

float *isax_query_binary_file_parallel_answering_chatzakis(const char *ifilename, int q_num, isax_index *index, float minimum_distance, query_result (*search_function)(ts_type *, ts_type *, isax_index *, node_list *, float, int, unsigned long long int *, unsigned long long int *))
{
    COUNT_INPUT_TIME_START
    if (comm_sz == 1)
        fprintf(stderr, ">>> Performing queries in file: %s\n", ifilename);
    ...
    float *results = malloc(sizeof(float) * q_num);
    // int maxparqathread = 1;
    pthread_t parQA_threads[maxparallelquerythread];
    parallel_query_thread_data parQA_data[maxparallelquerythread];

    long chunks = q_num / maxparallelquerythread;


    for (int i = 0; i < maxparallelquerythread; i++)
    {
        parQA_data[i].tid = i;
        parQA_data[i].from = i * chunks;

        if (i != maxparallelquerythread - 1)
            parQA_data[i].to = (i + 1) * chunks;
        else
            parQA_data[maxparallelquerythread - 1].to = q_num;

        parQA_data[i].results = results;
        parQA_data[i].ifile = ifile;
        parQA_data[i].minimum_distance = minimum_distance;
        parQA_data[i].search_function = search_function;
        parQA_data[i].queries = queries;
        parQA_data[i].nodelist = nodelist;
        parQA_data[i].index = index;

        parQA_data[i].for_all_queries_min_dist_counter_perthread = 0;
        parQA_data[i].for_all_queries_real_dist_counter_perthread = 0;
        parQA_data[i].time_per_thread = 0;
    }

    printf("MC - Data initializing, creating now the threads!\n");

    for (int i = 0; i < maxparallelquerythread; i++)
    {
        if (pthread_create(&parQA_threads[i], NULL, independent_query_answering_chatzakis, &parQA_data[i]))
        {
            perror("Could not create the parallel query answer threads");
            exit(EXIT_FAILURE);
        }
    }
    ...
    for (int i = 0; i < maxparallelquerythread; i++)
    {
        pthread_join(parQA_threads[i], NULL);
    }

    COUNT_QUERY_ANSWERING_TIME_END
    printf("MC - Threads Completed!\n");
    ...
    for (int i = 0; i < maxparallelquerythread; i++)
    {
        for_all_queries_real_dist_counter += parQA_data[i].for_all_queries_min_dist_counter_perthread;
        for_all_queries_min_dist_counter += parQA_data[i].for_all_queries_real_dist_counter_perthread;
    }

    if (comm_sz == 1)
    {
        print_query_statistics_results_chatzakis(q_num);
    }
...
node_group *create_node_groups(node_list *nodelist, int query_groups_num)
{
    // int query_groups_num = maxquerythread / 2;
    int total_buffers = nodelist->node_amount;
    int group_size = total_buffers / query_groups_num;

    node_group *groups = (node_group *)malloc(sizeof(query_group) * query_groups_num);

    for (int i = 0; i < query_groups_num; i++)
    {
        groups[i].from = i * group_size;

        if (i == query_groups_num - 1)
        {
            groups[i].to = total_buffers;
            groups[i].size = total_buffers - i * group_size;
        }
        else
        {
            groups[i].to = (i + 1) * group_size;
            groups[i].size = group_size;
        }

        groups[i].group_id = i;
        groups[i].nodelist = nodelist;
        groups[i].current_subtree_index = 0;
        groups[i].processed = 0;
        groups[i].pq = NULL;
    }
...
priority_query *load_priority_queries_from_file_chatzakis(isax_index *index, int q_num, FILE *ifile)
{
    ...
    priority_query *queries = (priority_query *)malloc(sizeof(priority_query) * q_num);
    ...
    for (int i = 0; i < q_num; i++)
    {
        ...
        // simulation:
        queries[i].priority = 0;
        if (i % 2 == 0)
        {
            queries[i].priority = 1;
        }
        else if (i % 3 == 0)
        {
            queries[i].priority = 2;
        }
    }
...
void merge_sort_priority_queries(priority_query *arr, int l, int r)
{
    if (l < r)
    {
        ...
        merge_sort_priority_queries(arr, l, m);
        merge_sort_priority_queries(arr, m + 1, r);

        merge_query_arr(arr, l, m, r);
    }
}

void sort_query_array_chatzakis(priority_query *queries, int q_num)
{
    // printf("[MC] - PQueries Before Sort:\n");
    // print_priority_queries_chatzakis(queries, q_num);

    merge_sort_priority_queries(queries, 0, q_num - 1);

    // printf("[MC] - PQueries After Sort:\n");
    // print_priority_queries_chatzakis(queries, q_num);
}

float *isax_query_binary_file_priority_query_answering_chatzakis(const char *ifilename, int q_num, isax_index *index, float minimum_distance, query_result (*search_function)(ts_type *, ts_type *, isax_index *, node_list *, float))
{
    COUNT_INPUT_TIME_START
    if (comm_sz == 1)
        fprintf(stderr, ">>> Performing queries in file: %s\n", ifilename);
    ...
    int q_loaded = 0;
    // ts_type *ts = malloc(sizeof(ts_type) * index->settings->timeseries_size);
    priority_query *queries = load_priority_queries_from_file_chatzakis(index, q_num, ifile);
    sort_query_array_chatzakis(queries, q_num);
    // Independent queries
    // ts_type * ts2 = malloc(sizeof(ts_type) * index->settings->timeseries_size);
    COUNT_INPUT_TIME_END
    // printf("[MC] - DEBUG - INPUT TIME END: After reading the queries calculations: %f\n", total_input_time);
    ...
    if (my_rank == MASTER)
    {
        printf("[MC] - Master: Priority Query Answering with %d threads.\n", maxquerythread);
    }
    ...
    while (q_loaded < q_num)
    {
        ...
        // Independent queries, KEEP ONLY LINE FREAD ts
        // fread(queries[q_loaded].query, sizeof(ts_type), index->settings->timeseries_size, ifile);
        ...
        query_result result = search_function(queries[q_loaded].query, paa, index, &nodelist, minimum_distance); // exact_search_ParISnew_inmemory_hybrid_ekosmas
        // printf("RES----> %f\n", result.distance);
...
float *isax_query_binary_file_dynamic_distributed_priority_query_answering_chatzakis(const char *ifilename, int q_num, isax_index *index, float minimum_distance, query_result (*search_function)(ts_type *, ts_type *, isax_index *, node_list *, float))
{
    COUNT_INPUT_TIME_START
    if (comm_sz == 1)
        fprintf(stderr, ">>> Performing queries in file: %s\n", ifilename);
    ...
    if (my_rank == MASTER)
    {
        printf("[MC] - Master: Dynamic Distributed Query Answering with %d threads.\n", maxquerythread);
    }
    ...
    // int initial_query_burst = 5; // how many queries the master sends to each node. int distributed_queries_initial_burst=1;
    int buffer_overload_nocomm = 1;
    int runs_before_checking = 1; // how many queries master answers before checking to send new queries to other nodes.
    ...
    struct timeval t1, t2;
    double elapsedCommunicationTime = 0;

    if (my_rank == MASTER)
    {
        for (int i = 1; i < comm_sz; i++)
        {
            for (int j = 0; j < distributed_queries_initial_burst; j++)
            {
                // COUNT_QUERY_ANSWERING_TIME_END
                COUNT_MPI_TIME_START
                // gettimeofday(&t1, NULL);
                MPI_Send(&q_loaded, 1, MPI_INT, i, DISTRIBUTED_QUERIES_SEND_QUERY, MPI_COMM_WORLD);
                // gettimeofday(&t2, NULL);
                // elapsedCommunicationTime += (t2.tv_sec - t1.tv_sec) * 1000.0;   // sec to ms
                // elapsedCommunicationTime += (t2.tv_usec - t1.tv_usec) / 1000.0; // us to ms
                COUNT_MPI_TIME_END
                // COUNT_QUERY_ANSWERING_TIME_START

                // printf("[MC] - Master send to node %d the query %d to answer.\n", i, q_loaded);

                q_loaded += buffer_overload_nocomm;
            }
        }
    }

    // printf("%f ms.\n", elapsedCommunicationTime);
    ...
    for (int i = 1; i < comm_sz; i++)
    {
        ...
        COUNT_MPI_TIME_START
        // gettimeofday(&t1, NULL);
        MPI_Irecv(&rec_message, 1, MPI_INT, i, DISTRIBUTED_QUERIES_REQUEST_QUERY, MPI_COMM_WORLD, &request[i]);
        // gettimeofday(&t2, NULL);
        // elapsedCommunicationTime += (t2.tv_sec - t1.tv_sec) * 1000.0;   // sec to ms
        // elapsedCommunicationTime += (t2.tv_usec - t1.tv_usec) / 1000.0; // us to ms
        COUNT_MPI_TIME_END
    ...
    while (1)
    {
        if (my_rank == MASTER)
        {
            ...
            results[q_loaded] = result.distance;
            // printf("[MC] - Master node answered the query %d with result %f.\n", q_loaded, results[q_loaded]);
            ...
            // update the statistics
            double time_for_the_query = query_answering_time - prev_query_answering_time;
            // printf("[MC] - MASTER answered the query %d with result %f in %f s.\n", q_loaded, results[q_loaded], time_for_the_query / 1000000);
            ...
            for (int i = 1; i < comm_sz; i++)
            {
                ...
                MPI_Test(&request[i], &ready, MPI_STATUS_IGNORE);
                ...
                if (ready)
                {
                    int q_2_send = q_loaded;
                    // COUNT_QUERY_ANSWERING_TIME_END
                    COUNT_MPI_TIME_START
                    // gettimeofday(&t1, NULL);
                    // MPI_Isend(&q_2_send, 1, MPI_INT, i, DISTRIBUTED_QUERIES_SEND_QUERY, MPI_COMM_WORLD, &send_request);
                    MPI_Send(&q_2_send, 1, MPI_INT, i, DISTRIBUTED_QUERIES_SEND_QUERY, MPI_COMM_WORLD);
                    //  gettimeofday(&t2, NULL);
                    //  elapsedCommunicationTime += (t2.tv_sec - t1.tv_sec) * 1000.0;   // sec to ms
                    //  elapsedCommunicationTime += (t2.tv_usec - t1.tv_usec) / 1000.0; // us to ms
                    COUNT_MPI_TIME_END
            ...
            if (q_loaded >= q_num)
            {
                ...
                for (int i = 1; i < comm_sz; i++)
                {
                    ...
                    COUNT_MPI_TIME_START
                    // gettimeofday(&t1, NULL);
                    MPI_Isend(&num_to_send, 1, MPI_INT, i, DISTRIBUTED_QUERIES_SEND_QUERY, MPI_COMM_WORLD, &send_request);
                    // MPI_Send (&num_to_send, 1, MPI_INT, i, DISTRIBUTED_QUERIES_SEND_QUERY, MPI_COMM_WORLD);
                    // gettimeofday(&t2, NULL);
                    // elapsedCommunicationTime += (t2.tv_sec - t1.tv_sec) * 1000.0;   // sec to ms
                    // elapsedCommunicationTime += (t2.tv_usec - t1.tv_usec) / 1000.0; // us to ms
                    COUNT_MPI_TIME_END
                    // COUNT_QUERY_ANSWERING_TIME_START
                    //  printf("[MC] - Master send to node %d the query %d to answer -- ending message.\n", i, num_to_send);
                }
        ...
        else
        {
            ...
            COUNT_MPI_TIME_START
            // gettimeofday(&t1, NULL);
            MPI_Recv(&q_loaded, 1, MPI_INT, MASTER, DISTRIBUTED_QUERIES_SEND_QUERY, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            // gettimeofday(&t2, NULL);
            // elapsedCommunicationTime += (t2.tv_sec - t1.tv_sec) * 1000.0;   // sec to ms
            // elapsedCommunicationTime += (t2.tv_usec - t1.tv_usec) / 1000.0; // us to ms
            COUNT_MPI_TIME_END
            ...
            COUNT_MPI_TIME_START
            // gettimeofday(&t1, NULL);
            MPI_Isend(&buffer_sent, 1, MPI_INT, MASTER, DISTRIBUTED_QUERIES_REQUEST_QUERY, MPI_COMM_WORLD, &send_request);
            // MPI_Send (&buffer_sent, 1, MPI_INT, MASTER, DISTRIBUTED_QUERIES_REQUEST_QUERY, MPI_COMM_WORLD);
            // gettimeofday(&t2, NULL);
            // elapsedCommunicationTime += (t2.tv_sec - t1.tv_sec) * 1000.0;   // sec to ms
            // elapsedCommunicationTime += (t2.tv_usec - t1.tv_usec) / 1000.0; // us to ms
            COUNT_MPI_TIME_END
    ...
    // printf("[MC] - Node %d spend %f time for communication.\n", my_rank, elapsedCommunicationTime / 1000);
...
/*REMOVED NEW
float *isax_query_binary_file_distributed_query_answering_chatzakis(const char *ifilename, int q_num, isax_index *index, float minimum_distance,
                                                                    query_result (*search_function)(ts_type *, ts_type *, isax_index *, node_list *, float))
{
    ...
    if (IS_LAST_NODE(my_rank, comm_sz))
    {
        to = q_num; // standard <<padding>>, baseline approach
    }
    else
    {
        to = (my_rank + 1) * sets;
    ...
    for (int i = from; i < to; i++)
    {
        ...
        // Parse ts and make PAA representation
        paa_from_ts(queries[i].query, paa, index->settings->paa_segments, index->settings->ts_values_per_paa_segment);
        query_result result = search_function(queries[i].query, paa, index, &nodelist, minimum_distance);
        results[i] = result.distance;
        // printf("[MC] - Node %d answering for query %d, result: %f\n", my_rank, q_loaded, results[q_loaded]);
...
float *isax_query_binary_file_distributed_makespan_query_answering_chatzakis(const char *ifilename, int q_num, isax_index *index, float minimum_distance,
                                                                             query_result (*search_function)(ts_type *, ts_type *, isax_index *, node_list *, float, communication_module_data *query_data),
                                                                             query_result (*approximate_search_func)(ts_type *, ts_type *, isax_index *index),
                                                                             dynamic_modes mode,
                                                                             char sort,
                                                                             char *estimations_filename,
                                                                             greedy_alg_comparing_field field_to_cmp)
{
    COUNT_INPUT_TIME_START
    ...
    int process_buffer_initial[comm_sz][distributed_queries_initial_burst];
    int process_buffer[comm_sz];
    ...
    while (1)
    {
        if (my_rank == MASTER)
        {
            if (mode == MODULE || mode == THREAD)
            {
                ...
                paa_from_ts(queries[query_to_keep_stats].query, paa, index->settings->paa_segments, index->settings->ts_values_per_paa_segment);
                query_result result = search_function(queries[query_to_keep_stats].query, paa, index, &nodelist, minimum_distance, &comm_data);
                results[query_to_keep_stats] = result.distance;

                COUNT_QUERY_ANSWERING_TIME_END
                COUNT_OUTPUT_TIME_END
                double time_for_the_query = query_answering_time - prev_query_answering_time;
                // printf("[MC] - MASTER answered the query %d with result %f in %f s.\n", query_to_keep_stats, results[query_to_keep_stats], time_for_the_query / 1000000);
                save_query_stats_chatzakis(query_to_keep_stats, result, time_for_the_query, queries_times);
                COUNT_OUTPUT_TIME_START
                COUNT_QUERY_ANSWERING_TIME_START
            }
        ...
        else
        {
            ...
            prev_query_answering_time = query_answering_time;
            paa_from_ts(queries[q_loaded].query, paa, index->settings->paa_segments, index->settings->ts_values_per_paa_segment);
            query_result result = search_function(queries[q_loaded].query, paa, index, &nodelist, minimum_distance, NULL); // exact_search_ParISnew_inmemory_hybrid_ekosmas
            ...
            COUNT_COMMUNICATION_TIME_START
            COUNT_MPI_TIME_START
            MPI_Isend(&buffer_sent, 1, MPI_INT, MASTER, DISTRIBUTED_QUERIES_REQUEST_QUERY, MPI_COMM_WORLD, &send_request[MASTER]);
            COUNT_MPI_TIME_END
            COUNT_COMMUNICATION_TIME_END
...
float *isax_query_binary_file_static_distributed_dress_query_answering_chatzakis(const char *ifilename, int q_num, isax_index *index, float minimum_distance,
                                                                                 query_result (*search_function)(ts_type *, ts_type *, isax_index *, node_list *, float),
                                                                                 query_result (*approximate_search_func)(ts_type *, ts_type *, isax_index *index),
                                                                                 char *estimations_filename,
                                                                                 char sort,
                                                                                 greedy_alg_comparing_field field_to_cmp)
{
    ...
    for (int curr_query = 0; curr_query < queries_counter; curr_query++)
    {
        ...
        query_result result = search_function(queries_to_ans[curr_query].query, paa, index, &nodelist, minimum_distance); // exact_search_ParISnew_inmemory_hybrid_ekosmas
        results[queries_to_ans[curr_query].id] = result.distance;
...
float *isax_query_binary_file_distributed_workstealing_chatzakis(const char *ifilename, int q_num, isax_index *index, float minimum_distance,
                                                                 query_result (*search_function)(int query_id, ts_type *ts, ts_type *paa, isax_index *index,
                                                                                                 node_list *nodelist, float minimum_distance,
                                                                 ...
                                                                 query_result (*ws_search_function)(int query_id, ts_type *ts, ts_type *paa, isax_index *index,
                                                                                                    node_list *nodelist , float minimum_distance,
                                                                                                    double (*estimation_func)(double), float bsf, int *batch_id))
{
    COUNT_INPUT_TIME_START

    FILE *ifile = fopen_validate_ds_file(ifilename, q_num, index->settings->ts_byte_size);
    dress_query *queries = load_dress_queries_from_file_chatzakis(index, q_num, ifile, NULL);
    double (*basis_func)(double) = initialize_basis_function(basis_function_filename);
    ...
    if (my_rank == MASTER)
    {
        printf("[MC] - Master: Distributed Query Answering (Workstealing) with %d threads.\n", maxquerythread);
    }
    ...
    for (int i = from; i < to; i++)
    {
        ...
        paa_from_ts(queries[i].query, paa, index->settings->paa_segments, index->settings->ts_values_per_paa_segment);
        query_result result = search_function(i, queries[i].query, paa, index, &nodelist, minimum_distance, basis_func);
        results[i] = result.distance;
    ...
    if (workstealing_chatzakis)
    {
        // workstealing(queries, q_num, index, minimum_distance, nodelist, ws_search_function, basis_func, results);
    }
    COUNT_QA_WORKSTEALING_TIME_END
